...
static int adicionar_aresta_direcionada(Grafo *grafo, int origem, int destino) {
...
int grafo_adicionar_aresta(Grafo *grafo, int origem, int destino) {
    if (grafo == NULL || origem < 0 || destino < 0) return 0;
    if ((size_t)origem >= grafo->quantidade_vertices || (size_t)destino >= grafo->quantidade_vertices) return 0;
    
    if (!adicionar_aresta_direcionada(grafo, origem, destino)) return 0;
    if (!adicionar_aresta_direcionada(grafo, destino, origem)) return 0;
...
Grafo* grafo_construir_de_dados(const DadosIdentificados *dados, size_t quantidade_vertices) {
    ...
    if (dados == NULL || quantidade_vertices == 0) return NULL;
